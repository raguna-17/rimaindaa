...
from memo.serializers import NoteSimpleSerializer, ReminderSerializer, NotificationSerializer
...
def test_notification_creation(note1, user1):
    ...
    notification = Notification.objects.create(user=user1, reminder=reminder, message='Test')
...
def test_notification_serializer(note1, user1):
    reminder = Reminder.objects.create(note=note1, remind_at=timezone.now())
    notification = Notification.objects.create(user=user1, reminder=reminder, message='Hi')
...
def test_create_reminder_valid(api_client, note1):
    ...
    response = api_client.post('/api/reminders/', data, format='json')  # ルーターに合わせる
